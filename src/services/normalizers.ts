...
export const normalizeVenue = (raw: any): Venue => {
  ...
  return {
    ...
    cover_image: raw.cover_image ?? courts.find((c: Court) => c.cover_image)?.cover_image ?? null,
...
export const normalizeBooking = (raw: any): Booking => ({
  ...
  is_cash_booking: Boolean(raw.is_cash_booking ?? false),
  is_cash_unpaid: Boolean(raw.is_cash_unpaid ?? false),
...
export const normalizeReview = (raw: any, venue?: Venue | null): Review => ({
  ...
  user_xp:
    raw.user?.xp !== undefined
      ? Number(raw.user.xp)
      : raw.user_xp !== undefined
        ? Number(raw.user_xp)
        : undefined,
  user_level:
    raw.user?.level !== undefined
      ? Number(raw.user.level)
      : raw.user_level !== undefined
        ? Number(raw.user_level)
        : undefined,
