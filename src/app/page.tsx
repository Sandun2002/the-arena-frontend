...
export default function Home() {
  return (
    <main className="w-full bg-surface-base overflow-x-hidden relative">
      {/* 1. Hero + Quick Search */}
      <Hero />
      ...
      <section className="py-24 relative bg-surface-raised/20 border-y border-default/50">
        <div className="container mx-auto px-4 max-w-7xl relative z-10">
          <div className="text-center mb-16">
            ...
            <p className="text-secondary max-w-2xl mx-auto">From discovery to game time in three simple steps.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8 relative">
            {/* Connector Line (Desktop only) */}
            <div className="hidden md:block absolute top-12 left-[16%] right-[16%] h-0.5 bg-gradient-to-r from-transparent via-subtle to-transparent border-t border-dashed border-subtle z-0" />

            {STEPS.map((step, idx) => (
              <div key={idx} className="relative z-10 flex flex-col items-center text-center group">
                <div className={`
                  w-24 h-24 rounded-3xl mb-8 flex items-center justify-center border-2 bg-surface-base transition-all duration-300 relative
                  ...
                  <step.icon size={40} weight="fill" className={`
                    ${step.color === 'emerald' ? 'text-emerald-500' :
                      step.color === 'blue' ? 'text-blue-500' :
                        'text-yellow-500'}
                  `} />
                  <div className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-surface-overlay border border-subtle flex items-center justify-center font-bold text-primary text-sm">
                    {idx + 1}
                  </div>
                </div>

                <h3 className="text-xl font-bold text-primary mb-3">{step.title}</h3>
                <p className="text-secondary text-sm max-w-xs leading-relaxed">{step.desc}</p>
              </div>
            ))}
          </div>
